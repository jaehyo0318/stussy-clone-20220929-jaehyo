...
class ProductApi {
    ...
    productDataUpdateRequest(formData) {
        $.ajax({
            async: false,
            type: "post",
            url: "/api/admin/products/modification",
...
class TopOptionService {
    ...
    loadPageMovement(productTotalCount) {
        ...
        this.pageMovement.createMoveButton(productTotalCount);
...
class PageMovement {
    ...
    createNumberButton(nowPage, productTotalCount) {
        ...
        let endIndex = startIndex + 4 <= this.getEndPageNuber(productTotalCount) ? startIndex + 4 : this.getEndPageNuber(productTotalCount);

        for(let i = startIndex; i <= endIndex; i++) {
            if(i == this.nowPage) {
                this.pageButtons.innerHTML += `
                    <a href="javascript:void(0)" class="a-selected"><li>${i}</li></a>
                `;
            }else {
                this.pageButtons.innerHTML += `
    ...
    createPostButton(nowPage, productTotalCount) {
        let maxPage = this.getEndPageNuber(productTotalCount);
...
class ElementService  {
    ...
    createProductDtl(productDetail) {
                        ...
                        <td colspan="3">
                            <form enctype="multipart/form-data">
                                <div class="product-img-inputs">
                                    ...
                                    <input type="file" class="file-input product-visible" name="file" multiple>                           
                                </div>
                            </form>
        ...
        productImgFileService.addImageFileEvenet();
